...
begin
  ...
  while (not Terminated) and (i < SourceFilesList.Count) do begin
    BatchSRF.LoadFromFile(SourceFilesList[i].FileName);
    WorkFile := TargetDirectory + SourceFilesList[i].ExtractedFileName('.xml');

    // Updating the progress dialog
    fStrBuf := 'Exporting "' + SourceFilesList[i].ExtractedFileName + '"...';
