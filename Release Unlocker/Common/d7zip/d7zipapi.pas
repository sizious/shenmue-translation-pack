...
{
  Modified by [big_fury]SiZiOUS : some enhancements for the
  Shenmue Translation Pack was performed.

  Search the "SiZ!" word inside the file to find what was modified...
}

unit D7zipAPI; {SiZ! : Renamed sevenzip to D7zipAPI to avoid confusing with the JCL unit}
{$ALIGN ON}
{$MINENUMSIZE 4}
...
type
  ...
  {SiZ! : Set the library file path...}
  procedure SevenZipSetLibraryFilePath(FilePath: TFileName);
...
procedure SevenZipSetLibraryFilePath(FilePath: TFileName);
begin
  SevenZipLibraryFilePath := FilePath;
end;
...
{SiZ! : Added this GetSevenZipLibraryName function.}
function GetSevenZipLibraryName: TFileName;
const
  VALID_NAMES: array[0..4] of string = (
    '7za.dll', '7zxa.dll', 'sevenzip.dll', '7zx.dll', '7z.dll'
  );
var
  i: Integer;

begin
  Result := SevenZipLibraryFilePath;
  if not FileExists(SevenZipLibraryFilePath) then
  begin
    // try to find the library in the current directory
    i := Low(VALID_NAMES);
    repeat
      Result := VALID_NAMES[i];
      Inc(i);
    until (i > High(VALID_NAMES)) or (FileExists(Result));
  end;
end;

function CreateInArchive(const classid: TGUID): I7zInArchive;
begin
  Result := T7zInArchive.Create(GetSevenZipLibraryName); {SiZ! : Removed the hard-coded string '7z.dll'}
  Result.ClassId := classid;
end;

function CreateOutArchive(const classid: TGUID): I7zOutArchive;
begin
  Result := T7zOutArchive.Create(GetSevenZipLibraryName); {SiZ! : Removed the hard-coded string '7z.dll'}
  Result.ClassId := classid;
end;
