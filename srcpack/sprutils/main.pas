...
const
  APP_VERSION = '2.0';
  COMPIL_DATE_TIME = 'July 17, 2008 @02:00 PM';
  ...
  if FindFirst(FilePath+'*.spr', faAnyFile, SR) = 0 then begin
    repeat
      if SR.Attr <> faDirectory then begin
        ...
        if (IsFileOpened(fName) = False) and (IsValidSpr(fName)) then begin
          SPRFileList.Add(fName);
          sprListBox.Items.Add(SR.Name);
        end
        else begin
          MsgBox(SR.Name+' is already opened or is not valid.', 'Error', MB_ICONERROR);
        end;
      end;
    until (FindNext(SR) <> 0);
  end;
