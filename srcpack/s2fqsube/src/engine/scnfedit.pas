...
begin
  ...
  try
    ...
    // on a pas affaire à un fichier de notre exporter
    ...
    begin
      ...
      begin
        try
          SubCode := LoopNode.Attributes['code'];
        except
          SubCode := '';
        end;
        try
          Subtitle := LoopNode.NodeValue;
        except
          Subtitle := '';
        end;
        Self.SubtitlesList[i].fCode := SubCode;
        Self.SubtitlesList[i].fText := Subtitle;
      end;
    end;
...
begin
  ...
  // open the file in read mode (to extract all infos on it)
  ...
  fScnfOriginalSize := fScnfUnknowValue1;
  ...
  if fScnfPosition <> 32 then begin
    ...
    BlockRead(F, Self.fCharacterModelUnknowValue, _SizeOf_Integer);
  end
  else begin
  ...
  {$IFDEF DEBUG} WriteLn(#13#10, 'Character Model Start Offset: ', fCharacterModelOffset);{$ENDIF}
  {$IFDEF DEBUG} WriteLn('Character Model Size: ', fCharacterModelUnknowValue);{$ENDIF}
...
procedure TSCNFEditor.SaveToFile(FileName: TFileName);
const
  WORK_BUFFER_SIZE = 16384;
  // END_STR_SIGN: array[0..2] of Char = (#$0, TABLE_CHAR_CODE, TABLE_STR_ENTRY_BEGIN);
  END_STR_SIGN: array[0..2] of Char = (#$A1, #$D7, #0);
  
var
  F_src, F_dest: file;
  Buf: array[0..WORK_BUFFER_SIZE-1] of Char;
  i, j, k, ScnfSize, SectionPos, MdcSize, BufSize: Integer;
...
begin
  ...
  if fScnfPosition > 32 then begin
    MdcSize := FileSize(F_src) - (32 + 40 + fScnfOriginalSize + null_bytes_length(fScnfOriginalSize));
    CopyFileBlock(F_src, F_dest, fCharacterModelOffset, MdcSize);
  end;

  // keeping SCNF position, if needed
    ...
    if fCharsMod then begin
      for j := 1 to Length(SubtitlesList[i].fText) do begin
        ...
        if ((StrBuf = '¡') and (SubtitlesList[i].fText[j+1] = 'õ')) or ((StrBuf = 'õ') and (SubtitlesList[i].fText[j-1] = '¡')) then begin
          ...
          tempByte := processChar(StrBuf); //Sending char...
        end;
  ...
  // keeping MCDx position & writing model
  if fScnfPosition = 32 then begin
    SectionPos := FileSize(F_dest) - 16;
    MdcSize := FileSize(F_src) - (32 + 40 + fScnfOriginalSize + null_bytes_length(fScnfOriginalSize));
    CopyFileBlock(F_src, F_dest, fCharacterModelOffset, MdcSize);
  ...
  if fScnfPosition = 32 then begin
    ...
    BlockWrite(F_dest, fCharacterModelUnknowValue, _SizeOf_Integer);
  end
  else begin
    // Footer unknow value (4 bytes)
    BlockWrite(F_dest, fFooterUnknowValue, _SizeOf_Integer);
