...
begin
  MsgBox('Version ' + APP_VERSION + #13#10 + 'Modified by Manic' + #13#10 + COMPIL_DATE_TIME, 'Information', MB_ICONINFORMATION);
end;
...
begin
  ...
  with odMain do begin
    if Execute then begin
      if SCNFEditor.ImportSubtitlesToFile(FileName) then begin
        lbSubSelectClick(Self);
        SetModified(True);
        AddDebug('Subtitles imported from ' + ExtractFileName(FileName));
      end
      else begin
...
begin
  ...
  // Loading chars list
  fCSVLoaded := loadCharsList('chars_list.csv');
  fCharsMod := fCSVLoaded;
  charsModMenu1.Enabled := fCSVLoaded;
  charsModMenu1.Checked := fCSVLoaded;
end;
...
begin
  if SCNFEditor.FileLoaded then begin
    SetStatus('Saving...');
    SCNFEditor.SaveToFile(SCNFEditor.GetLoadedFileName, True);
...
begin
  ...
  if SCNFEditor.FileLoaded then begin
    with sdMain do begin
      ...
      if Execute then begin
        SCNFEditor.SaveToFile(FileName, False);
        SetModified(False);
      end;
    end;
  end; {else
