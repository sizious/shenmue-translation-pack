...
file_begin, file_end, lowest_srf:Integer; Check, jap_switch:Boolean;
...
begin
        ...
        begin
                ...
                begin
                        if (i=0) and (LowerCase(ExtractFileExt(files_infos[i])) = '.srf') then
                        begin
                                jap_switch := True;
                        end;
                ...
                for i:=0 to srf_list.Count-1 do
                begin
                        //Finding first and last audio file of the current SRF
                        if (jap_switch = False) and (srf_list[i] = lowest_srf) then
                        begin
                                file_begin := 0;
                                file_end := srf_list[i] - 1;
                        end
                        else if (jap_switch = True) and (srf_list[i] = lowest_srf) then
                        begin
                                file_begin := 1;
                                file_end :=srf_list[i+1] - 1;
                        end
                        else
                        begin
                                if jap_switch = False then
                                begin
                                ...
                                begin
                                        if i = srf_list.Count-1 then
                                        begin
                                                file_begin := srf_list[i] + 1;
