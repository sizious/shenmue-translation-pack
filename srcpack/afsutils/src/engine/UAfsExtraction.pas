...
uses
  Windows, ActiveX, Forms, Classes, SysUtils, Math, progress;
...
begin
  ...
  for i := 0 to afsMainQueue.FileIndex.Count - 1 do begin
    if not Terminated then begin
      ...
      if afsMain.FileDate[intBuf] <> '' then begin
        FDate := StrToDateTime(afsMain.FileDate[intBuf]);
        FAge := DateTimeToFileDate(FDate);
        FileSetDate(outFileName, FAge);
      end;
